...
export function variable_to_string(variable: Variable, tables_done: Variable[] = []): string
{
	...
	{
		case VariableKind.Nil: return 'nil'
		case VariableKind.Boolean: return variable.boolean ? 'true' : 'false'
		case VariableKind.Number: return variable.number?.toString() ?? '0'
		case VariableKind.String: return variable.string ?? ''
		case VariableKind.Function: return `<Function ${ variable.function_id ?? 'nil' }>`
		case VariableKind.NativeFunction: return `<Function ${ variable.native_function?.name ?? 'nil' }>`
...
function type(_: Engine, variable: Variable): Variable[]
{
	return [make_string(variable.data_type)]
}

function inext(_: Engine, table: Variable, previous_index: Variable): Variable[]
{
...
function next(_: Engine, variable: Variable, start_index?: Variable): Variable[]
{
...
function string_char(_: Engine, ...chars: Variable[]): Variable[]
{
	...
	const s = String.fromCharCode(...chars.map(c => c.number))
...
function string_format(_: Engine, format: Variable, ...args: Variable[]): Variable[]
{
	...
	for (const char of format.string)
	{
		if (is_format)
		{
			result += string_format_helper(char, args_iterator)
			is_format = false
			continue;
		}
...
function math_max(_: Engine, ...args: Variable[]): Variable[]
{
	...
	const max = args.reduce((acc, x) => Math.max(acc, x.number), Number.NEGATIVE_INFINITY)
...
{
	assertPopulatedArray<VariableNumber>(args, unary(assertVariableKind, VariableKind.Number));
	const min = args.reduce((acc, x) => Math.min(acc, x.number), Number.POSITIVE_INFINITY)
